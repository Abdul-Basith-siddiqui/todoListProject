...
//mongodb+srv://NovaKing:<password>@cluster0.ae46b4r.mongodb.net/?retryWrites=true&w=majority
mongoose.connect(
  `mongodb+srv://NovaKing:${process.env.PASSWORD}@cluster0.ae46b4r.mongodb.net/todoListDB`,
...
//model
const Item = mongoose.model("Item", itemsSchema); //model is collection
...
app.get("/", function (req, res) {
  ...
  Item.find({}, (err, items) => {
    //finding DB using find to print on screen

    if (items.length === 0) {
      // if in our DB array is empty, equal to 0 then inserting
...
app.post("/delete", (req, res) => {
  ...
  if (listName === day) {
    Item.findByIdAndRemove(checkedBox, (err) => {
      //mongoose - removing the document by id
      if (err) console.log(err);
      // if callback is not written the this method wont run ,it simply search the id
      else console.log("removed");
      res.redirect("/");
    });
  } else {
    //D:usefindOneandupdate ({filter},  {update}, functon)  in update we are using $pull opertator form mongoDB to pull specific document- obj
...
app.get("/:customListName", function (req, res) {
  ...
  // res.render(search);
  // console.log(search);
  List.findOne({ name: search }, (err, isThere) => {
    //(findOne) will give us an object // but (find) will give us array of object
    if (err) console.log(err);
    else if (isThere) {
      //what user search is already there then do this
      // console.log("exits");
      //display
      res.render("list", {
        listTitle: isThere.name,
        newListItems: isThere.items,
      });
    } else {
...
// app.post("/link",function(req,res){   my expriment
//   const link= req.body.link;
//  res.redirect("/"+link);
// });
// app.get("/"+link, function(req,res){
//     res.render("list",{links:link});
// });
